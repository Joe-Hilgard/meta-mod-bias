...
WLS-MRA (single slope): Meta-analysis tests the moderator effect after PET adjustment for publication bias. PET slope is fixed across groups. This is implemented via the metafor::rma() function.

WLS-MRA (variable slope): Meta-analysis tests the moderator effect after PET adjustment for publication bias. The PET slope is allowed to vary across groups. This is implemented via the metafor::rma() function.

Selection modeling: 
This is implemented via the weightr::weightfunct() function (Coburn & Vevea, CITATION NEEDED). We modeled a single publication bias cutpoint at *p* = .05. The resulting mismatch between the selection model and the true publication model is deliberately chosen to emulate probable model misspecification in real-world application.
