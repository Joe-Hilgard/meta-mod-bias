...
Publication bias is harmful because it inflates effect sizes, making small effects look big and null effects look like something.

In addition to these harms, publication bias may make it harder to detect moderators.
In this simulation, we explore how publication bias may impair power to detect moderators.
...
P-curve and p-uniform, while popular in social psychology, do not allow for moderation analysis. That said, these methods are special cases of the more general technique of selection modeling (Hedges & Vevea? Vevea & Hedges?), which can accomodate moderators.

Trim and fill does not allow for moderation analysis, and it's not a very good adjustment either.

PET meta-regression attempts to partial out the effect of publication bias as a small-study effect. It fits a regression to the relationship between effect size and precision. The intercept, representing a hypothetical study with perfect precision, is interpreted as the bias-adjusted effect size.

Stanley and Doucouliagos (2014) suggest that PET-PEESE meta-regression can be applied in a multiple regression format. They suggest simulataneously modeling the small-study effect with PET or PEESE and modeling variation according to the moderator.
However, they do not elaborate on how to choose this model (should it be PET or PEESE?) and they do not use Monte Carlo simulation to examine its efficacy.
...
It is not clear whether one should attempt a PEESE-RMA.

Relevant citation is Stanley & Doucouliagos 2014, "Meta-regression approximations to reduce publication selection bias", see their equation 10. They don't really elaborate on it or test its efficiency or bias -- they just say "throw the moderator on in".
